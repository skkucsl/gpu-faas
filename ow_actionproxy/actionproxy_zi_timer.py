...
class ActionRunner:
    ...
    def init(self, message):
        ...
        global pstart
        global pend
        pstart = os.popen("date +%s%N").read().replace('\n','')
        if prep():
            try:
                ...
                return False
        pend = os.popen("date +%s%N").read().replace('\n','')
    ...
    def verify(self):
        global vstart
        global vend
        vstart = os.popen("date +%s%N").read().replace('\n','')
        if (os.path.isfile(self.lib) and os.access(self.lib, os.R_OK)):
            ...
            self.func = getattr(FUNC_DL, output)
            self.func.restype = ctypes.c_double
            vend = os.popen("date +%s%N").read().replace('\n','')
            return True
        else:
            td = -1.0
            return False
    ...
    def run(self, args, env):
        ...
        o = "{ \"vstart\": " + vstart + ", \"vend\": " + vend + ", \"pstart\": " + pstart + ", \"pend\": " + pend + ", \"init_on_lib\": " + str(init_lib) + ", \"func_time\": " + str(func_time) + "}"
...
def init(message=None):
    global istart
    istart = os.popen("date +%s%N").read().replace('\n','')
